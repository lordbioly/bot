...
def cmd_bot(who, msg):
	...
	s = botsession.think(msg);
	systoone(who, _(s))
	
def cmd_plan(who, msg):
	'/plan - wyswietlanie planu lekcji (dzien tygodnia w formacie 1-5, pn -pt lub pelna nazwa)'	
	
	
	if (msg == ''):
		dzien = time.strftime("%a")
		if (dzien == "Mon"):
			plik = open('plan/1.txt')
			try:
				tekst = plik.read()
			finally:
				plik.close()
			systoone(who, _(tekst))
		elif (dzien == "Tue"):
			plik = open('plan/2.txt')
			try:
				tekst = plik.read()
			finally:
				plik.close()
	...
	elif (msg == 'poniedzialek' or msg == 'pn' or msg == '1'):
		plik = open('plan/1.txt')
		try:
			tekst = plik.read()
		finally:
			plik.close()
		systoone(who, _(tekst))
	elif (msg == 'wtorek' or msg == 'wt' or msg == '2'):
		plik = open('plan/2.txt')
		try:
			tekst = plik.read()
		finally:
			plik.close()
		systoone(who, _(tekst))
	elif (msg == 'sroda' or msg == 'sr' or msg == '3'):
		plik = open('plan/3.txt')
		try:
			tekst = plik.read()
		finally:
			plik.close()
		systoone(who, _(tekst))	
	elif (msg == 'czwartek' or msg == 'czw' or msg == '4'):
		plik = open('plan/4.txt')
		try:
			tekst = plik.read()
		finally:
			plik.close()
		systoone(who, _(tekst))
	elif (msg == 'piatek' or msg == 'pt' or msg == '5'):
		plik = open('plan/5.txt')
		try:
			tekst = plik.read()
		finally:
		...
		systoone(who, _('Zla skladnia polecenia lub podales zly dzien tygodnia'))

def cmd_czas(who, msg):
	'/czas - podaje aktualny czas'
	czas =  time.strftime("%H:%M:%S")
	systoone(who, _(czas))
	
def cmd_data(who, msg):
	'/data - podaje aktualna date'
